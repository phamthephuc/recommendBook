...
allItem = getListData("select bid from book");
data_from_database = pd.DataFrame(index=allUser,columns=allItem)
data_from_database[:] = 0;

allBying = getListObject("SELECT id_user, id_pro FROM orders INNER JOIN detailorder ON orders.id_order = detailorder.id_order WHERE id_user != 0 GROUP BY id_user, id_pro")  
for i in allBying:
